...
contract MultiSigVault {
    ...
    function _verifyMultiSignature2(FunctionInfo calldata _txn, uint256 _nonce,bytes[] calldata _multiSignature) private {
        require(_nonce > nonce, "nonce already used");
        uint256 count = _multiSignature.length;
        require(count >= _threshold, "not enough signers");
        bytes32 digest = _processFunctionInfo(_txn, _nonce);
    ...
    function _processFunctionInfo(FunctionInfo calldata _txn, uint256 _nonce) private pure returns(bytes32 _digest){
